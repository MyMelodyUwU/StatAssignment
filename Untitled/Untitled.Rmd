...
- Investigation of x. 
- We will go through y
- To create conclusin of z
...
```{r plot2d, echo=FALSE, fig.align='center'}
plot2 <- passwords %>%
    filter(rank < 100) %>%
    group_by(interaction(strength,offline_crack_sec)) %>%
    # mutate(Banana = paste(type,ifelse(shortcut == "Yes", "With Shourtcut","Without Shortcut"))) %>%
...
```{r scatterAttempt, echo=FALSE}
  name <- names(passwords)
  # Filter me?
  # fit0 <- lm(offline_crack_sec ~ value, data = passwords)
  passPlot3 <- passwords %>% 
  filter(strength < 20)
  plot3 <- ggplot(passPlot3, aes(x = rank, y = strength)) + 
  geom_point() + 
